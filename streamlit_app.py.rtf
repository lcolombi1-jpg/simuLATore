...
st.set_page_config(page_title="Latin & Greek Quiz", page_icon="\uc0\u55356 \u57307 \u65039 ")\
\
st.title("\uc0\u55356 \u57307 \u65039  Esercitazione di Lingue Classiche")\
...
if 'domande' not in st.session_state:\
    st.session_state.domande = [\
        ...
        \{\
            ...
            "opzioni": ["Rom\uc0\u257 ", "In Rom\u257 ", "Romam", "Romae"],\
            "corretta": "Romae"\
        \},\
        \{\
            "id": 7,\
...
if st.button("Invia e Verifica Risultati", type="primary"):\
    ...
    for q in st.session_state.domande:\
        ...
        if risposta_studente == q['corretta']:\
            punti += 1\
            st.success(f"Domanda \{q['id']\}: Esatto! \uc0\u9989 ")\
        elif risposta_studente is None:\
            st.warning(f"Domanda \{q['id']\}: Non hai risposto. La corretta era '\{q['corretta']\}'. \uc0\u9888 \u65039 ")\
        else:\
            st.error(f"Domanda \{q['id']\}: Errato. La risposta corretta era '\{q['corretta']\}'. \uc0\u10060 ")\
    \
    st.divider()\
    ...
    if punti == len(st.session_state.domande):\
        st.balloons()\
        st.success("Optime! Conoscenza perfetta della lingua latina! \uc0\u55356 \u57307 \u65039 ")}